"""Fetch Amvera run logs (redacted). Usage: python scripts/amvera_logs.py [hours] [query]"""
...
def main() -> None:
    # Compat: amvera_logs.py [query] OR amvera_logs.py [hours] [query]
    hours = 2.0
    q = "*"
    if len(sys.argv) == 2:
        try:
    ...
    text = re.sub(r"(token[=:\s]+)\S+", r"\1***", text, flags=re.I)
    text = re.sub(r"(password[=:\s]+)\S+", r"\1***", text, flags=re.I)
