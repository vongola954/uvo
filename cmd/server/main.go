...
func main() {
	...
	deps := &webapi.Deps{
		...
		MaxOn: maxC.Enabled(), Version: "2.7.8",
	}
	...
	logrus.Infof("UVO 2.7.8 on %s:%d (db=%s prod=%v bot=%s max=%v)", cfg.WebHost, cfg.WebPort, cfg.DBDriver, cfg.IsProduction(), cfg.BotMode, maxC.Enabled())
